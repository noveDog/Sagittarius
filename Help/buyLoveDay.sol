...
interface loveDay{
    function addUserbuy(address _addr, uint256 _value) external;
}
contract Ownable {
...
contract buyToken is Ownable{
    ...
    loveDay loveDayContract;
    
    
    function init(address _SAGtoken, address _rp, address _USDT, address _router02, address _lovDayContract) public onlyOwner() {
        SAGToken = _SAGtoken;
        RP = relationship(_rp);
        USDT = _USDT;
        router02 = IdexRouter02(_router02);
        loveDayContract = loveDay(_loveDayContract);
    ...
    function buy(address _father, uint256 _amount) public {
        ...
        _buy(_amount);
        _sendLoveDayGif(msg.sender, _amount);
    }
    ...
    function _sendLoveDayGif(address _user, uint256 _amount) internal {
        address[] memory path = new address[](2);
        path[0] = USDT;
        path[1] = SAGToken;
        
        uint256 _value = router02.getAmountsOut(_amount, path);
        loveDayContract.addUserbuy(_user, _value);
    }
