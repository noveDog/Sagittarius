...
contract SGRPool is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (pool.hasDeposit > pool.maxDeposit){
            GCGPerBlock = pool.maxDeposit.mul(pool.rewardMultiple).mul(pool.GCGPerBlock).add(pool.hasDeposit.sub(pool.maxDeposit).mul(pool.GCGPerBlock)).div(pool.hasDeposit);
        }else{
            GCGPerBlock = pool.rewardMultiple.mul(pool.GCGPerBlock);
        }



        uint256 multiplier = getMultiplier(pool.lastRewardBlock, currentBlock);
        uint256 GCGReward = multiplier.mul(GCGPerBlock);
